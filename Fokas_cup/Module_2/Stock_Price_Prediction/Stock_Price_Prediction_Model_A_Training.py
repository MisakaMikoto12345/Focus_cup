...
from LSTM_A_Module import LSTM_A
...
def data_transform(x_train, y_train, x_test, y_test):
    ...
    y_train_gru = torch.Tensor(y_train)
    y_test_gru = torch.Tensor(y_test)
...
def model_training(input_dim,hidden_dim,num_layers,output_dim,num_epochs,x_train,y_train_lstm):
    model = LSTM_A(input_dim=input_dim, hidden_dim=hidden_dim, num_layers=num_layers, output_dim=output_dim)
...
if __name__ == '__main__':
    ...
    x_train, x_test, y_train_lstm, y_test_lstm, y_train_gru, y_test_gru = data_transform(x_train,y_train,x_test,y_test)
    ...
    #反归一化
    y_train_pred = scaler[0].inverse_transform(y_train_pred.detach().numpy())
    y_train = scaler[0].inverse_transform(y_train_lstm.detach().numpy())

    y_test_pred = scaler[0].inverse_transform(y_test_pred.detach().numpy())
    y_test = scaler[0].inverse_transform(y_test_lstm.detach().numpy())

    path = r'../../Data/Model/LSTM_A.pt'
    ...
    original = scaler[0].inverse_transform(price['收盘价'].values.reshape(-1, 1))
    ...
    py.offline.plot(fig, filename=r'..\..\Data\Model\result_A.html', auto_open=False)
